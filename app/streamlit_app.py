    ...
    if load and repo_url:
        with st.spinner("Loading existing index…"):
            try:
                ...
                st.session_state.messages = []
                st.session_state.indexed_repo = repo_url
                st.success(f"✅ Loaded: `{repo_url}`")
            except Exception as e:
                st.error(f"❌ No index found. Please click 🚀 Index first.")

    st.divider()

    if st.session_state.get("qa_instance") is not None:
        st.caption("**Active repo:**")
        st.code(st.session_state.get("indexed_repo", ""), language=None)
...
if not qa:
    ...
    for msg in messages:
        with st.chat_message(msg["role"]):
            ...
            if msg.get("sources"):
                ...
                with st.expander(f"📂 {source_count} source(s)"):
    ...
    user_input = pending or st.chat_input("Ask about the codebase…")
    ...
    if user_input:
        ...
        with st.chat_message("assistant"):
            ...
            if result.sources:
                ...
                with st.expander(f"📂 {result_count} source(s)"):
